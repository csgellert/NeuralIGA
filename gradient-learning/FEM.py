...
def GaussQuadrature(x1,x2,i,p,knotvector):
    ...
    return Ke, Fe
def u_star(x,xA=0,xB=10,uA=0,uB=0):
    w1 = x-xA
    w2 = xB-x
...
def evaluate(u,t,p,xx):
    ...
    return sum
def ValuePoint(u,t,p,x):
    sum = 0
    for i in range(0,len(u)):
        Ni = disctance(x)*B(x,p,i,t)
        ui = u[i]
        Ni = ui*Ni
        sum +=Ni
    #sum = np.zeros(xx.shape)
    sum+=u_star(x)
    return sum
def ValuePointD(u,t,p,x):
    sum = 0
    for i in range(0,len(u)):
        Ni = disctance(x)*dBdXi(x,p,i,t) + distanceDerivative(x)*B(x,p,i,t)
        ui = u[i]
        Ni = ui*Ni
        sum +=Ni
    #sum = np.zeros(xx.shape)
    sum+=0#u_star(x)
    return sum
def ValuePointD2(u,t,p,x):
    sum = 0
    for i in range(0,len(u)):
        Ni = distance2ndDerivative(x)*B(x,p,i,t) + 2* distanceDerivative(x)*dBdXi(x,p,i,t) + disctance(x)*d2BdXi2(x,p,i,t)
        ui = u[i]
        Ni = ui*Ni
        sum +=Ni
    #sum = np.zeros(xx.shape)
