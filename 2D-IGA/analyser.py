...
for division in test_values:
    ...
    for elemx in range(p,p+xDivision+1):
        for elemy in range(q,q+xDivision+1):
            Ke,Fe = FEM.elemantBspline(p,q,knotvector_u,knotvector_w,None,elemx,elemy,NControl_u,NControl_w,weigths,ctrlpts)
    ...
    accuracy.append(FEM.calculateErrorBspline(Surfacepoints,ctrlpts,result,NControl_u,NControl_w,weigths,knotvector_u,knotvector_w,p,q))
fig,ax = plt.subplots()
