...
class KANLinear(nn.Module):
    ...
    For each edge (out, in) it learns a univariate function represented by a
    piecewise-linear spline on a uniform grid, and sums contributions over input
    dimensions. Optionally includes a standard linear term on top of a base
    activation (as in common KAN implementations).
    """
    ...
    def __init__(
        ...
        # Spline coefficients for piecewise-linear interpolation on a uniform grid.
    ...
    def _spline_interpolate(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if self.spline_order == "linear":
            ...
            v0 = _gather_at(idx0)
            v1 = _gather_at(idx1)
            t_ = t.unsqueeze(1)  # (B, 1, in)
            v = v0 * (1.0 - t_) + v1 * t_  # (B, out, in)
        else:
            # Catmull–Rom cubic interpolation (C1-continuous).
            # Uses 4 neighboring control points: p0, p1, p2, p3.
...
class KAN(nn.Module):
    ...
    Uses the same `architecture` list convention as the other models in this file.
    """
...
def load_test_model(name, type, params = {}):
    if type == "SIREN":
        ...
        # No pretrained KAN weights are shipped by default; this is here to keep
        # the API symmetric with the other networks.
        architecture = params.get("architecture", [2, 64, 64, 1])
        grid_size = params.get("grid_size", 16)
        grid_range = params.get("grid_range", (-1.0, 1.0))
        use_base = params.get("use_base", True)
        base_activation = params.get("base_activation", "silu")
        spline_init_scale = params.get("spline_init_scale", 1e-2)
        model = KAN(
