...
# -------------- Multi Sided domains -----------------
class ForkSIREN(nn.Module):
    def __init__(self, num_sides,mutual_architecture, subarchitecture,outermost_linear=False,
                 first_omega_0=60, hidden_omega_0=60):
        super().__init__()
        if len(mutual_architecture) < 2:
            raise ValueError("mutual_architecture must have at least input and one output width")
        if len(subarchitecture) < 2:
            raise ValueError("subarchitecture must have at least input and one output width")
        if num_sides < 1:
            raise ValueError("num_sides must be >= 1")

        self.architecture = mutual_architecture
        ...
        shared_layers = []
        first_layer = SineLayer(mutual_architecture[0], mutual_architecture[1], is_first=True, omega_0=first_omega_0)
        shared_layers.append(first_layer)
        for i in range(len(mutual_architecture) - 2):
            shared_layers.append(
                SineLayer(
                    mutual_architecture[i+1],
                    mutual_architecture[i + 2],
                    omega_0= hidden_omega_0,
                )
            )
        self.shared_net = nn.Sequential(*shared_layers)

        shared_out_features = mutual_architecture[-1]
        if subarchitecture[0] == shared_out_features:
            sub_arch = list(subarchitecture)
        elif subarchitecture[0] == mutual_architecture[0]:
            sub_arch = [shared_out_features] + list(subarchitecture[1:])
        else:
            raise ValueError(
                f"subarchitecture input ({subarchitecture[0]}) must match shared output "
                f"({shared_out_features}) or original input ({mutual_architecture[0]})"
            )
        ...
        # Keep a single iterable for utilities that inspect model.net.
        #self.net = nn.ModuleList([self.shared_net, self.subnets])
       

    def forward(self, coords):
        mutual_output = self.shared_net(coords)
        side_outputs = []
        for subnet in self.subnets:
            side_outputs.append(subnet(mutual_output))
        return torch.cat(side_outputs, dim=1)

class Parallel_SIREN(nn.Module):
    def __init__(self, num_sides, subarchitecture,outermost_linear=False,
                 first_omega_0=60, hidden_omega_0=60):
        super().__init__()
        if len(subarchitecture) < 2:
            raise ValueError("subarchitecture must have at least input and one output width")
