...
import torch
import PDE_testcases as testcases
...
TRANSFORM =  None  # Options:  "hollig", None
...
GAMMA_HOLLIG = 1  # Controls the smoothness for Höllig weight function
...
def distance_with_derivative_vect_trasformed(x,y,model,transform=None):
    ...
    dx = grds[0][:,0]
    dy = grds[0][:,1]
    if transform and transform.lower() == "hollig":
...
def plotDisctancefunction(model):
    N = 200
    x_values = np.linspace(testcases.DOMAIN["x1"]-0.1, testcases.DOMAIN["x2"]+0.1, N)
    y_values = np.linspace(testcases.DOMAIN["y1"]-0.1, testcases.DOMAIN["y2"]+0.1, N)
